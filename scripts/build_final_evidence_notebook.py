#!/usr/bin/env python
"""Build the 'final paper evidence' notebook (Q1 submission gap closure).

The notebook orchestrates the real CLI entry points only — it duplicates
**no** scientific algorithm.  It runs the remaining evidence that the deep
audit flagged as still open:

  A. SOTA-style baselines   (OddSHAP log-odds + ShaplEIG-style GP, N=20)
  B. N=50 finite-population reruns (wine + air, standard budget)
  C. Instrumented matched-budget curves (actual coalition/model call counts)
  D. Ablation N=20 with tier-4 width/coverage (regenerates the lost CSV)
  E. Regime semantics N>=20 (audit: pilot N=1-2 too small)
  F. Frontier completion: fp probe at K = 150k / 200k (nominal 1-delta)
  G. Adversarial stress test of the finite-population certificate
  H. Coverage calibration at tight epsilon (0.05 / 0.1 / 0.2 / 0.5)

Every section is env-var overridable and independently skippable:

  GAS_SKIP=A..H        comma list to skip (e.g. "GAS_SKIP=C,E")
  SOTA_N=20 ABL_N=20 REG_N=20 FP_N=50 PROBE_BUDGETS=150000,200000
  STRESS_TRIALS=200 COV_TRIALS=200

Smoke test (fast):
  GAS_SKIP=C,D,E,H SOTA_N=2 FP_N=1 PROBE_BUDGETS=20000 STRESS_TRIALS=10 COV_TRIALS=10

Usage:
    python scripts/build_final_evidence_notebook.py
"""
...
OUT = ROOT / "main_results" / "RUN_FINAL_EVIDENCE.ipynb"
...
cells.append(md("## 0b. Configuration"))
...
cells.append(md("## A. SOTA-style baselines (OddSHAP + ShaplEIG, N=20)"))
...
cells.append(md("## B. N=50 finite-population reruns (wine + air)"))
...
cells.append(md("## C. Instrumented matched-budget curves"))
...
cells.append(md("## D. Ablation N=20 (regenerates tier-4 width/coverage)"))
...
cells.append(md("## E. Regime semantics N≥20"))
...
cells.append(md("## F. Frontier completion — fp probe at K = 150k / 200k"))
...
cells.append(md("## G. Adversarial stress test of the fp certificate"))
...
cells.append(md("## H. Coverage calibration at tight ε"))
...
cells.append(md("## I. Summary of produced artifacts"))
