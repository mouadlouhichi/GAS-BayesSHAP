#!/usr/bin/env python
"""Build the 'final certification' notebook (audit blockers 1+2 closure).

The latest audit's two hard blockers:
  1. no sign-certified features / no convergence on the main real-data runs;
  2. finite-population intervals are width-tight but NOT nominal 1-delta at
     the standard budget (fraction_at_nominal_level = 0.0).

The single-instance wine probe already showed the way: at K=200,000 the
coupon thresholds close and the run returns status CERTIFIED with
`certificate_at_nominal_level=True` and a sign-certified feature (validated
against exact).  This notebook generalises that to MULTIPLE real instances
(wine + air) so the paper can claim multi-instance nominal certification at
the characterised frontier cost, with every certified sign validated.

Sections:
  A. Wine  — finite-population, K=200k, N=3 instances
  B. Air   — finite-population, K=200k, N=3 instances
  C. Summary table + exact-sign validation report

Env overrides: N_INST (default 3), BUDGET (default 200000), GAS_SKIP.

Usage:
    python scripts/build_final_certification_notebook.py
"""
...
OUT = ROOT / "main_results" / "RUN_FINAL_CERTIFICATION.ipynb"
...
cells.append(md("## A. Wine — nominal certification at the frontier budget"))
...
cells.append(md("## B. Per-dataset detail"))
...
cells.append(md("## C. Summary + validation report"))
